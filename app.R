...
server <- function(input, output, session) {
  ...
  output$download_report <- downloadHandler(
    ...
    content = function(file) {
      ...
      jb <- if (n >= 20) tseries::jarque.test(x)$p.value else NA
      ...
      <ul>
        <li>Shapiro-Wilk p-value: ", round(sh, 5), 
        " → <b>", keputusan(sh), "</b></li>
        
        <li>Lilliefors p-value: ", round(li, 5), 
        " → <b>", keputusan(li), "</b></li>
        
        <li>Jarque-Bera p-value: ", round(jb, 5), 
        " → <b>", keputusan(jb), "</b></li>
      </ul>
      ...
      ",
        ifelse(
          mean(c(sh > alpha, li > alpha, jb > alpha), na.rm = TRUE) >= 0.5,
